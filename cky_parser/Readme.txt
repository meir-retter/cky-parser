...
parser.py should be run using Python 3. The appropriate shebang is in place.

Q4:

I found the words at the fringe of the tree and rare-fied them using a recursive function (as often happens when trees are involved).

Q5:
One change I made in this implementation was using a dummy slot at the start of sentences, which made the indexing more consistent in style with our in-class version.
Here were the results for running eval_parser on the prediction file created by my implementation of CKY:
...
With the larger number of nonterminals to deal with, the time taken to run CKY after the vertical markovization was considerably more than before.

As one can observe, vertical markovization generally improved performance (notably for SBAR) but there were exceptions. For example:

PRON had unchanged performance.
NP+NOUN had worsened precision.
NP+DET had worsened recall.
NP had worsened precision.
ADVP+ADV had worsened F-score.
